...
pub(crate) struct BuiltinParser<E>(
...
impl<E> BuiltinParser<E>
where
    E: MetaError,
{
    pub fn new(stream: TokenStream, last_span: Option<Span>) -> Self {
        Self(stream.into_iter(), last_span, PhantomData)
    }
    ...
    /// parse item
    pub fn parse_root_item<V>(
        &mut self,
        token: Option<TokenTree>,
        visitor: V,
    ) -> Result<V::Output, BuiltinParserError<E>>
    where
        V: MetaVisitor<Domain = TokenTree>,
    {
        match token {
            ...
            Some(TokenTree::Punct(punct)) if _is_path_delimiter(&punct) => {
                ...
                visitor.visit_path(Some(delimiter), None, BuiltinContSource { parser: self })
            }

            Some(TokenTree::Ident(ident)) => visitor.visit_path(
                Some(ident.span()),
        ...
        V: MetaVisitor<Domain = TokenTree>,
    {
        match token {
            ...
            Some(token) => visitor
                .visit_value(Some(token.span()), token)
                .map_err(BuiltinParserError::Error),
        }
    }

    pub fn parse_item_cont<V>(
        &mut self,
        token: Option<TokenTree>,
        visitor: V,
    ) -> Result<V::Output, BuiltinParserError<E>>
        ...
        V: MetaVisitor<Domain = TokenTree>,
    {
        match token {
            ...
            Some(TokenTree::Punct(punct)) if punct.as_char() == '=' => {
                ...
                visitor
                    .visit_value(Some(value.span()), value)
                    .map_err(BuiltinParserError::Error)
            }
